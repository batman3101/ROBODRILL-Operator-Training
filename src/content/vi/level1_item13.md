...
<p class="subtitle">Level 1 - Item 13<br>Dry run và Single Block</p>
</div>
<div class="cover-meta">
<strong>Đối tượng</strong><span>Người vận hành mới, nhân viên chất lượng và nhân viên bảo trì thiết bị</span>
<strong>Mục tiêu</strong><span>Thực hành dry run, single block và kiểm tra override trước lần chạy đầu.</span>
...
# 13. Dry run và Single Block

Mục này giúp học viên mới hiểu và giải thích an toàn nội dung Dry run và Single Block. Thực hành dry run, single block và kiểm tra override trước lần chạy đầu. Người vận hành không chỉ học tên nút mà phải đọc trạng thái máy, dự đoán hành động tiếp theo và nhận biết vị trí nguy hiểm trước khi thao tác. Mọi thao tác được thực hiện theo thứ tự kiểm tra, chọn, thực hiện, quan sát và ghi lại.
...
- Thực hành dry run, single block và kiểm tra override trước lần chạy đầu.
...
| --- | --- | --- |
| Hiểu | Giải thích mục đích của Dry run và Single Block | Dùng ít nhất 3 thuật ngữ chính |
...
Thời điểm nguy hiểm nhất trong Dry run và Single Block là khi người vận hành chạy bước tiếp theo mà chưa đọc trạng thái máy. Trục chính, chuyển động trục, thay dao, nhập tọa độ và chạy chương trình đều có thể biến thao tác trên màn hình thành chuyển động thật. Học viên phải nhìn đồng thời số trên màn hình và vị trí dao thật.
...
![Hình tham khảo Dry run và Single Block](../../assets/diagrams/training_flow.svg)

<p class="caption">Hình 13-1. Hình tham khảo cho bài Dry run và Single Block. Nguồn và quyền sử dụng được ghi trong tài liệu nguồn hình ảnh.</p>
...
![Mockup màn hình CNC và bảng điều khiển cho Dry run và Single Block](../../assets/diagrams/control/level1_item13_control.svg)

<p class="caption">Hình 13-2. Mockup màn hình CNC và bảng điều khiển cần đọc trong bài Dry run và Single Block.</p>
...
![Sơ đồ học chức năng Dry run và Single Block](../../assets/diagrams/training/level1_item13_study.svg)

<p class="caption">Hình 13-3. Sơ đồ học cho Dry run và Single Block, gồm thứ tự thao tác và chuyển động thật.</p>
...
Điểm cốt lõi của Dry run và Single Block không chỉ là nhớ quy trình, mà là hiểu vì sao quy trình đó tồn tại. ROBODRILL là máy nhỏ và nhanh, nên một lần bỏ sót kiểm tra cũng có thể dẫn tới va chạm hoặc lỗi chất lượng. Vì vậy ở cấp cơ bản, thói quen quan sát an toàn quan trọng hơn tốc độ thao tác.
...
4. Tìm nút hoặc menu cần dùng cho Dry run và Single Block và nói tên của nó.
...
Thực hành: Tại vị trí đào tạo do giảng viên chỉ định, học viên nói to quy trình Dry run và Single Block, sau đó thực hiện từng bước và dừng lại sau mỗi bước. Sau mỗi bước, học viên mô tả trạng thái màn hình và trạng thái máy thật.
</div>
...
3. Nói 5 hạng mục cần kiểm tra trước Dry run và Single Block.
...
1. Việc đầu tiên cần kiểm tra trước Dry run và Single Block là gì?
...
1. Viết 5 hạng mục cần kiểm tra trước Dry run và Single Block.
...
Giảng viên không chỉ kiểm tra học viên có nhớ quy trình hay không. Cần kiểm tra học viên có giải thích được vì sao phải dừng và xác nhận ở từng bước hay không. Dry run và Single Block liên kết với bài tiếp theo, nên sau thực hành phải luôn để lại câu hỏi và ghi nhận.

## Liên kết với mục tiếp theo

Bài tiếp theo là 14 Quản lý dung dịch cắt và phoi. Thói quen kiểm tra trạng thái, dừng an toàn và ghi nhận trong bài này sẽ tiếp tục được dùng ở bài sau.
