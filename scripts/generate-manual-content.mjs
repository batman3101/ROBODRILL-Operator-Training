...
function controlMockupPath(item) {
  return `../../assets/diagrams/control/level1_item${item.id}_control.svg`;
}

function koControlMockupFigure(item, title) {
  return `![${title} CNC 화면 및 조작 패널 mockup](${controlMockupPath(item)})

<p class="caption">그림 ${Number(item.id)}-2. ${title} 수업에서 읽어야 할 CNC 화면 값과 확인해야 할 조작 패널 위치를 함께 표시한 mockup.</p>
...
function viControlMockupFigure(item, title) {
  return `![Mockup màn hình CNC và bảng điều khiển cho ${title}](${controlMockupPath(item)})

<p class="caption">Hình ${Number(item.id)}-2. Mockup màn hình CNC và bảng điều khiển cần đọc trong bài ${title}.</p>
...
<p class="caption">그림 ${Number(item.id)}-3. ${title}의 조작 순서와 실제 움직임을 함께 보는 학습용 도해.</p>
...
<p class="caption">Hình ${Number(item.id)}-3. Sơ đồ học cho ${title}, gồm thứ tự thao tác và chuyển động thật.</p>
...
${koControlMockupFigure(item, title)}
...
${viControlMockupFigure(item, title)}
