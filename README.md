...
각 항목에는 실제 설비 이미지와 함께 CNC 화면/조작 패널 mockup, 축 이동 방향, 핸들 회전 방향, 좌표/오프셋 표, 공구교환 흐름처럼 실제 학습에 필요한 기능 설명 도해가 포함됩니다. CNC 화면/패널 mockup은 `scripts/generate-control-mockups.mjs`에서 생성해 `src/assets/diagrams/control/`에 저장하고, 동작 설명 도해는 `scripts/generate-training-diagrams.mjs`에서 생성해 `src/assets/diagrams/training/`에 저장합니다. 초보자 학습성을 위해 도해 가독성을 우선하며, 이미지가 늘어난 만큼 페이지 수가 증가하는 것을 허용합니다.
